...
const (
	...
	modelIDMinimaxM25     = "minimax-m2.5"
	modelIDMinimaxM25Free = "minimax-m2.5-free"
	modelIDClaudePrefix   = "claude-"
)
...
func main() {
	target := flag.String("target", "", "catalog target: openai, zen or opencode-go")
...
func generateZen(providers map[string]providerData) ([]byte, error) {
	...
	entries := normalizeEntries(providerIDZen, src.Models, src.NPM, shouldIncludeZenModel)
...
func generateOpenCodeGo(providers map[string]providerData) ([]byte, error) {
	...
	entries := normalizeEntries(providerIDOpenCodeGo, src.Models, src.NPM, shouldIncludeOpenCodeGoModel)
...
func normalizeEntries(_ string, models map[string]modelData, _ string, include func(modelData) bool) []entry {
...
func shouldIncludeZenModel(m modelData) bool {
	if strings.HasPrefix(m.ID, modelIDClaudePrefix) {
		return false
	}

	return m.ID != modelIDMinimaxM25 && m.ID != modelIDMinimaxM25Free
}

func shouldIncludeOpenCodeGoModel(m modelData) bool {
	return m.ID != modelIDMinimaxM25
}
