...
func main() {
	target := flag.String("target", "", "catalog target: openai, zen or anthropic")
...
func generateOpenAI(providers map[string]providerData) ([]byte, error) {
	...
	all := normalizeEntries(src.Models, func(modelData) bool { return true })
	chatgpt := normalizeEntries(src.Models, isChatGPTModel)
...
func generateZen(providers map[string]providerData) ([]byte, error) {
	...
	entries := normalizeEntries(src.Models, func(modelData) bool { return true })
...
func normalizeEntries(models map[string]modelData, include func(modelData) bool) []entry {
